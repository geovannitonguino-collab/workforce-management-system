...
import { calculateDailySummary, calculatePayroll, getCurrentPeriod, getToday, formatCurrency, formatHours } from '@/lib/calculations';
import { DollarSign, TrendingUp, Clock, CalendarCheck } from 'lucide-react';
    ...
    <div className="stat-card space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" /> Work Hours</p>
        ...
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1"><CalendarCheck className="h-3 w-3" /> Paid Leave</p>
      ...
      <p className="text-xs text-muted-foreground">Rate: {formatCurrency(employee.hourlyRate)}/hr</p>
    </div>
  );
}
